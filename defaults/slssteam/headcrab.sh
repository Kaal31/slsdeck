#!/usr/bin/env bash
	...
	InstallArchDeps(){
	    ...
	    if [ ${#to_install[@]} -ne 0 ]; then
	        if steamoscheck || [ -f /etc/steamos-release ]; then
	            echo "SteamOS detected: Skipping rootfs pacman install (system is read-only). Using user space tools."
	            return 0
	        fi
	        echo "Installing missing Arch packages: ${to_install[*]}"
	        if ! sudo pacman -S --noconfirm "${to_install[@]}"; then
	            echo "Warning: pacman install failed or skipped."
	        fi
	    fi
		fi
	}

    ...
    const doTokeer = async () => {
        ...
        try {
            ...
            const r = await setupAndVerifyTokeer(appid, (status) => {
                ...
                if (lower.includes("runtime version"))
                    percent = 18;
                else if (lower.includes("checking required proton"))
                    percent = 38;
                else if (lower.includes("merging steam launch options"))
                    percent = 62;
                else if (lower.includes("checking the game setup"))
                    percent = 76;
                else if (lower.includes("creating the proton prefix"))
                    percent = 84;
...
function TokeerSection() {
    ...
    const restoreActivationPanel = async (generation = ticketGenerationRef.current) => {
        ...
        try {
            ...
            if (generation === ticketGenerationRef.current)
                setRestoringSelectors(!restored && !!selectorLayoutRef.current);
        }
    };
    const abortTicketChain = (reason) => {
    ...
    const continueUbisoftTicket = async () => {
        ...
        try {
            ...
            if (!tokenMessageId) {
                setBusy("Finding the Ubisoft token request…");
                ...
                if (!token.success || !token.found || !token.path || !token.filename) {
                    setAutomationStage("waiting-token");
                    setMessage(token.error || "No fresh Ubisoft token request was found yet. Run the game and retry Continue.");
                    return;
                }

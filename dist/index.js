...
async function waitForTicketContext(fromUrl = "", timeoutMs = 20000, expectedAppid = 0, existingChannelIds = [], onTicketDiscovered, expectedGameName = "") {
    ...
    const normalizeGame = (value) => String(value || "").normalize("NFKD").replace(/[®™©'’‘`´]/g, "").replace(/[^a-z0-9]+/gi, " ").trim().toLowerCase();
            ...
            if (!lastTicketUrl && sidebarBefore.size) {
                try {
                    ...
                    if (created) {
                        ...
                        const discovered = { found: false, opened: true, ...ticketIdentity(lastTicketUrl), parentChannelId: TOKEER_TICKET_PARENT_CHANNEL_ID, ticketChannelId: created.id, error: "Ticket thread found; waiting for its setup commands…" };
                        try {
    ...
    while (Date.now() < focusDeadline && !focused?.ok) {
        ...
        if (!focused?.ok) {
            ...
            if (tab?.webSocketDebuggerUrl) {
                ...
                const named = boxes.filter((node) => /(?:message|write|send|нап(?:исать|ишите)|nachricht|mensaje|écrire|scrivi|escrever|wiadomo)/i.test(String(node?.name?.value || "")));
...
function TokeerSection({ headless = false, activationRequest } = {}) {
    ...
    const refreshAvailability = async (force = false, announce = force) => {
        if (announce)
            setMessage(ticketChainActive() ? "Refreshing the live vault, then restoring your private ticket…" : "Refreshing live vault and game availability…");
    ...
    const runAutomation = async (ctx, resume, generation = ticketGenerationRef.current) => {
        ...
        try {
            ...
            if (!ubisoftTicket) {
                ...
                const completeNonUbisoftActivation = async (completionStage, tracked) => {
                    if (completionStage === "checking-game") {
                        ...
                        setBusy("Launching the game and waiting for Steam confirmation…");
            ...
            if (ubisoftTicket) {
                ...
                setBusy("Applying the hosted Ubisoft package…");
    ...
    if (headless) {
        ...
        return SP_JSX.jsxs("div", { style: { border: "1px solid rgba(202,168,255,.28)", borderRadius: 8, padding: 8, background: "rgba(202,168,255,.06)" }, children: [SP_JSX.jsxs("div", { style: { fontSize: 13, fontWeight: 600, marginBottom: 4 }, children: ["Tokeer activation", activationRequest?.remaining !== undefined ? ` · ${activationRequest.remaining}${activationRequest.total !== undefined ? ` / ${activationRequest.total}` : ""} keys available` : ""] }), SP_JSX.jsx("div", { style: { fontSize: 11, opacity: .72, lineHeight: 1.45, marginBottom: 7 }, children: resumable ? `Saved activation for ${savedGame || "this game"}. The same ticket and progress are available in Advanced → Tokeer helper.` : `Runs the complete ${selectedUbisoft ? "Ubisoft" : "Tokeer"} activation chain and shares its ticket state with Advanced → Tokeer helper.` }), SP_JSX.jsx(DFL.ButtonItem, { layout: "below", disabled: cancelling || ((!!busy || automationRunningRef.current) && !ticketProcessRunning), onClick: ticketProcessRunning ? cancelTicket : (resumable ? resumeFromFixes : () => setHeadlessArmed(true)), children: cancelling ? (busy || "Cancelling ticket…") : ticketProcessRunning ? "Cancel ticket" : busy || automationRunningRef.current ? (busy || `Activation: ${automationStage.replace("-", " ")}`) : ticket?.url ? (isUbisoftTicket ? "Continue Ubisoft ticket" : "Continue ticket") : "Activate with Tokeer" }), !!ticket?.url && !ticketProcessRunning && !cancelling && SP_JSX.jsx(DFL.ButtonItem, { layout: "below", disabled: !!busy, onClick: cancelTicket, children: "Cancel ticket" }), automationStage !== "idle" && SP_JSX.jsxs("div", { style: { fontSize: 10, marginTop: 6, opacity: .78 }, children: ["Stage: ", SP_JSX.jsx("b", { children: automationStage.replace("-", " ") }), tlxSubmitted ? " · TLX1 submitted" : ""] }), message && SP_JSX.jsx("div", { style: { fontSize: 10, marginTop: 6, lineHeight: 1.4, color: automationError ? "#ff7b72" : "inherit" }, children: message })] });
    }
...
function FixPicker({ appid, onReload, onClose }) {
    ...
    const doArchiveToggle = async () => {
        if (!currentBuildId) {
            setMsg("No pinned build to archive — pin or install a build first.");
            return;
        }
        setBusy("archive");
        try {
    ...
    const neraiList = (check.neraiFixes || []);
    neraiList.forEach((e, i) => {
        ...
        rows.push({
            ...
            info: { status: 200, available: true, url: e.url, file: e.file, badge: online ? "online" : "bypass" },
    ...
    const renderFixFlow = (key) => {
        ...
        const rawPhasePercent = fixState.percent;
    ...
    return (SP_JSX.jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 8, padding: "4px 0" }, children: [pinned && (SP_JSX.jsxs("div", { style: { fontSize: 11, opacity: 0.75, lineHeight: 1.5 }, children: [SP_JSX.jsxs("div", { children: [tokeerApplied
                ...
                return (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsxs("div", { style: { fontSize: 12, fontWeight: 600, opacity: 0.85, marginTop: 2 }, children: ["lua.tools fixes (", cat.length, ")"] }), cat.map((fix, i) => {
                    ...
                    : archived
                        ...
                        : currentBuildId ? `Archive game snapshot (${currentBuildId})` : "Archive game snapshot" }), (!dlcOwnedOnly || (!added && isInLibrary(appid))) && (SP_JSX.jsx(DFL.DialogButton, { style: { fontSize: 12, padding: "5px 8px" }, disabled: working || !!awaiting || dlcDownload?.status === "resolving" || dlcDownload?.status === "downloading", onClick: doDlcContent, children: busy === "dlcdepot" ? "Working…" : dlcDownload?.status === "resolving" || dlcDownload?.status === "downloading"
                    ? `Downloading DLC… ${Math.max(1, Math.min(99, Math.round(dlcDownload.percent || 1)))}%`
                    : "Get DLC files + unlock" })), dlcDownload && (SP_JSX.jsxs("div", { style: { padding: "2px 2px 7px" }, children: [SP_JSX.jsxs("div", { style: { display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 4 }, children: [SP_JSX.jsxs("span", { children: ["DLC depots \u00B7 ", dlcDownload.status] }), SP_JSX.jsxs("span", { children: [dlcDownload.status === "done" ? 100 : Math.max(1, Math.min(99, Math.round(dlcDownload.percent || 1))), "%"] })] }), SP_JSX.jsx("div", { style: { height: 7, borderRadius: 4, overflow: "hidden", background: "rgba(255,255,255,.14)" }, children: SP_JSX.jsx("div", { style: {
                                height: "100%",
                    ...
                    : dlcU[kind]?.installed
                        ...
                        : `Unlock ${UNLOCKER_LABEL[kind]}` }, kind)) : null), hv?.found && (SP_JSX.jsxs("div", { style: { border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, padding: 8 }, children: [SP_JSX.jsxs("div", { style: { fontSize: 13, fontWeight: 600, marginBottom: 4 }, children: ["HVAuto crack \u00B7 build ", hv.buildid || "?"] }), SP_JSX.jsxs("div", { style: { fontSize: 11, opacity: 0.65, marginBottom: 6 }, children: ["Compatible build: ", hv.buildid || "unknown", ". This does not change your version pin; use Specific build first when a downgrade is needed."] }), SP_JSX.jsx(DFL.DialogButton, { style: { fontSize: 12, padding: "5px 8px" }, disabled: working || !!awaiting, onClick: doHv, children: busy === "catalog:hv"
                            ? "Preparing / applying HV crack…"
                            : `Apply HV crack${hv.status === "older" ? " · older target build" : ""}` }), renderFixFlow("catalog:hv")] })), crak?.found && (SP_JSX.jsxs("div", { style: { border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, padding: 8 }, children: [SP_JSX.jsxs("div", { style: { fontSize: 13, fontWeight: 600, marginBottom: 4 }, children: ["CrakFiles \u00B7 build ", crak.buildid || "?"] }), !!crak.badges?.length && (SP_JSX.jsx("div", { style: { marginBottom: 4 }, children: crak.badges.map((b, i) => SP_JSX.jsx(BadgeChip, { badge: b }, i)) })), SP_JSX.jsxs("div", { style: { fontSize: 11, opacity: 0.65, marginBottom: 6 }, children: ["Compatible build: ", crak.buildid || "unknown", ". This does not change your version pin; use Specific build first when a downgrade is needed."] }), SP_JSX.jsx(DFL.DialogButton, { style: { fontSize: 12, padding: "5px 8px" }, disabled: working || !!awaiting, onClick: doCrak, children: busy === "catalog:crak"
                        ...
                        catch { /* */ } }, children: "Re-open download page" })] })), msg && (SP_JSX.jsxs(DFL.Focusable, { style: { display: "flex", flexDirection: "column", gap: 5 }, children: [SP_JSX.jsx("div", { style: {
                            fontSize: 11,
                            lineHeight: 1.4,
                            opacity: 0.82,
                            padding: "0 2px",
        ...
        poll = setInterval(async () => {
            ...
            try {
                ...
                if (!appearedInLibrary && !wasInLibrary && isInLibrary(appid)) {
                    ...
                    setStatus$1("Added — available in Steam");
                    ...
                    let notify = true;
                    try {
                        notify = (await getNotifyGameAdd()).enabled !== false;
                    }
                    catch { /* default on */ }
                    if (notify) {
                        const early = ((_a = window).__slsdeckEarlyAddNotified || (_a.__slsdeckEarlyAddNotified = new Set()));
                        early.add(appid);
                    ...
                    setStatus$1(st.slssteam || st.status === "reconciling"
                ...
                if (["done", "failed", "cancelled"].includes(st.status || "")) {
                    ...
                    if (st.status === "done") {
                        ...
                        if (action === "add")
                            await reinject(appid);
                        setStatus$1(st.liveReady ? "Added — available in Steam" : "Added — restart Steam to finish provisioning");
                    }
                    else {
                        markSlsAddPending(appid, false);
        ...
        try {
            ...
            if (f.has_manifest) {
                ...
                if (pin.changed !== false) {
                    setStatus$1("Exact fix build pinned — uninstall and reinstall the game, then press this fix again");
                    return;
                }
            }
...
function InstalledSection({ refreshToken, onChanged }) {
    ...
    const [reloadOnPurge, setReloadOnPurge] = SP_REACT.useState(true);
    const load = async () => {
        setLoading(true);
        try {
    ...
    const confirmPurge = () => {
        DFL.showModal(SP_JSX.jsx(DFL.ConfirmModal, { strTitle: "Purge all added games?", strDescription: `This removes ALL ${apps.length} added game(s) from SLSsteam — every registration and lua manifest. It does NOT delete installed game files. ${reloadOnPurge ? "Steam and any running game will close, then Steam will restart after the purge." : "Steam will stay open and Moon will refresh the library in-session."} This cannot be undone (restore a backup if needed).`, strOKButtonText: reloadOnPurge ? "Purge and reload Steam" : "Purge all", onOK: async () => {
                try {
...
async function evalString(wsUrl, expression, timeoutMs = 2500) {
    return new Promise((resolve) => {
        ...
        sock.onopen = () => {
            try {
                sock.send(JSON.stringify({ id: 91, method: "Runtime.evaluate", params: { expression, returnByValue: true, awaitPromise: true } }));
            }
            catch {
                finish("");
            }
        ...
        sock.onmessage = (ev) => {
            try {
                ...
                if (msg?.id === 91)
                    finish(typeof msg?.result?.result?.value === "string" ? msg.result.result.value : "");
            }
            catch { /* */ }
        };
        sock.onerror = () => finish("");
...
async function cdpCommand(wsUrl, method, params = {}, timeoutMs = 4000) {
    return new Promise((resolve) => {
        let done = false;
        let sock;
        const finish = (value) => {
...
async function findSteamdbTab(appid) {
    ...
    for (const tab of candidates) {
        ...
        if (href.includes(`steamdb.info/app/${appid}/patchnotes`))
            return { ...tab, url: href };
    }
    return null;
}
...
async function readBuildsFromDom(wsUrl) {
...
async function closeTab(id, port = 8080) {
    if (!id)
        return;
    try {
...
async function findTab$1(urlPart) {
    ...
    for (const tab of candidates) {
        const live = await evalOnTab$2(tab.webSocketDebuggerUrl, "String(location.href||'')", 2500);
        if ((live || tab.url || "").includes(urlPart))
            return { ...tab, url: live || tab.url };
    }
    return null;
}
...
function GameToolsSection() {
    ...
    const [pinnedBuild, setPinnedBuild] = SP_REACT.useState("");
    const [pinnedDepotCount, setPinnedDepotCount] = SP_REACT.useState(0);
    ...
    const refreshPinStatus = SP_REACT.useCallback(() => {
        if (appid == null)
            return;
        getPinStatus(appid)
            .then((r) => {
    ...
    const openRollback = async () => {
        ...
        DFL.showModal(SP_JSX.jsx(PickerModal, { title: "Reset files", subtitle: "Pick a build. Steam re-downloads the files that differ, resetting them to that build.", items: pitems, onPick: (it) => {
                const e = items.find((x) => x.id === it.key);
                if (!e || e.current) {
                    setNote("Already on that build.");
                    return;
                }
                DFL.showModal(SP_JSX.jsx(DFL.ConfirmModal, { strTitle: "Reset this game's files?", strDescription: `Pin ${e.buildid ? `build ${e.buildid}` : "this build"} and let Steam re-download the changed files. Reversible — pin the latest again anytime.`, strOKButtonText: "Reset files", onOK: () => run("rollback", () => buildHistoryRollback(appid, e.id), (r) => {
                        if (!r.success)
    ...
    return (SP_JSX.jsxs(DFL.PanelSection, { title: "Actions", children: [SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsxs("div", { style: { fontSize: 12, opacity: 0.85, padding: "2px 0" }, children: ["Proton: ", SP_JSX.jsx("span", { style: { fontWeight: 600 }, children: protonLabel })] }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ButtonItem, { layout: "below", disabled: !!busy, onClick: pickProton, children: busy === "proton" ? "Working…" : "Change Proton version" }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ButtonItem, { layout: "below", disabled: !!busy, onClick: () => run("backup", () => backupGameSaves(appid, ""), (r) => r.success
                        ...
                        await pollDdlOnce();
                        startDdl();
                    }, children: ddlActive && ddl?.op === "dlc" ? "Downloading DLC…" : "Blind download content DLC (DepotDownloader)" }) })), depotdl && (SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx("div", { style: { fontSize: 11, opacity: 0.65, padding: "0 2px 4px", lineHeight: 1.4 }, children: "Downloads every keyed depot with no checks \u2014 it does not verify that a depot is DLC (language packs and base depots can be included), does not match your platform, and does not skip files you already have. For the filtered version use \u201CGet DLC files + unlock\u201D in Fixes." }) })), depotdl && ddl && ddl.op !== "build" && (SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsxs("div", { style: { width: "100%", padding: "2px 0" }, children: [SP_JSX.jsxs("div", { style: { fontSize: 12, marginBottom: 4, display: "flex", justifyContent: "space-between" }, children: [SP_JSX.jsxs("span", { children: [ddl.op === "dlc" ? "DLC-candidate depots" : "Build depots", " \u00B7 ", ddl.status] }), SP_JSX.jsx("span", { style: { opacity: 0.8 }, children: ddl.status === "downloading" || ddl.status === "resolving" ? `${Math.max(1, Math.min(99, Math.round(ddl.percent || 1)))}%` : ddl.status === "done" ? "100%" : "" })] }), SP_JSX.jsx("div", { style: { height: 6, background: "rgba(255,255,255,0.15)", borderRadius: 3, overflow: "hidden" }, children: SP_JSX.jsx("div", { style: {
                                    height: "100%",
                                    width: `${ddl.status === "done" ? 100 : Math.max(1, Math.min(99, Math.round(ddl.percent || 1)))}%`,
                                    background: ddl.status === "failed" ? "#d9534f" : ddl.status === "done" ? "#5cb85c" : "#4a90d9",
                                    transition: "width 0.3s",
                                } }) }), ddl.error && (SP_JSX.jsx("div", { style: { fontSize: 11, color: ddl.status === "failed" ? "#f0ad4e" : "#8fbf8f", marginTop: 4, lineHeight: 1.4 }, children: ddl.error })), !!ddl.plannedDepots?.length && SP_JSX.jsxs("div", { style: { marginTop: 7, padding: 7, borderRadius: 6, background: "rgba(0,0,0,.18)", fontSize: 10, lineHeight: 1.5 }, children: [SP_JSX.jsxs("div", { style: { fontWeight: 700, marginBottom: 3 }, children: ["Depots ", ddl.depotDone || 0, "/", ddl.depotTotal || ddl.plannedDepots.length] }), ddl.plannedDepots.map((d) => {
                                    const m = ddl.depotMetadata?.[d.depot];
                                    const label = m?.kind === "dlc" ? `DLC ${m.dlcAppid || ""}` : m?.kind === "shared" ? `shared from ${m.fromAppid || "app"}` : m?.kind === "base-or-shared" ? "base/shared" : d.kind === "dlc-candidate" ? "DLC candidate" : "build";
                                    return SP_JSX.jsxs("div", { style: { display: "flex", justifyContent: "space-between", gap: 8, color: ddl.failedDepots?.includes(d.depot) ? "#f0ad4e" : ddl.completedDepots?.includes(d.depot) ? "#8fd49a" : ddl.currentDepot === d.depot ? "#72c7ff" : "rgba(255,255,255,.72)" }, children: [SP_JSX.jsxs("span", { children: [ddl.currentDepot === d.depot ? "▶ " : ddl.completedDepots?.includes(d.depot) ? "✓ " : ddl.failedDepots?.includes(d.depot) ? "! " : "", "Depot ", d.depot, m?.name ? ` · ${m.name}` : ""] }), SP_JSX.jsxs("span", { style: { opacity: .65 }, children: [label, m?.os ? ` · ${m.os}` : "", m?.language ? ` · ${m.language}` : "", " \u00B7 GID ", d.manifest] })] }, d.depot);
                                }), ddl.enrichmentStatus === "running" && SP_JSX.jsx("div", { style: { marginTop: 5, opacity: .65 }, children: "Enriching depot relationships in the background\u2026" }), ddl.op === "dlc" && SP_JSX.jsx("div", { style: { marginTop: 5, color: "#d7b7ff", opacity: .85 }, children: "Candidate means the depot came from the full game bundle; Steam app-info is still required to prove that it belongs to a DLC." })] }), ddl.status === "done" && !ddl.error && (SP_JSX.jsx("div", { style: { fontSize: 11, color: "#8fbf8f", marginTop: 4 }, children: "Done \u2014 files placed in the game folder. Restart Steam if needed." }))] }) })), depotdl && (SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ButtonItem, { layout: "below", disabled: !!busy, onClick: async () => {
                        setBusy("bp");
                        let builds = [];
                        try {
                            const r = await bpListBuilds(appid);
                    ...
                    }, children: busy === "archive" ? "Archiving…" : "Archive game snapshot…" }) })), steamless?.supported && (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ButtonItem, { layout: "below", disabled: !!busy, onClick: doSteamless, children: busy === "steamless"
                                ? "Working…"
 ...
 */
function SlsSteamCompact({ status, statusChecked, onRefreshStatus, }) {
    ...
    if (status?.installed && !working && (onLibraryGamePage || (onStoreGamePage && !showReinstall)))
        return null;
...
function DependenciesSection() {
    ...
    const [tokeerInstalled, setTokeerInstalled] = SP_REACT.useState(false);
    const [protonStatus, setProtonStatus] = SP_REACT.useState(null);
    const [ubisoftPackages, setUbisoftPackages] = SP_REACT.useState(null);
    const [cloudStatus, setCloudStatus] = SP_REACT.useState(null);
    ...
    const refresh = async () => {
        ...
        try {
            ...
            if (proton.installed)
                setN("tokeerProton", "GE-Proton10-34 installed");
            else if (proton.partial)
                setN("tokeerProton", "partial installation detected — repair required");
        }
        catch { /* */ }
        try {
            ...
            if (packages.installed)
                setN("ubisoftPackages", "hosted packages installed");
        }
        catch { /* */ }
        try {
            ...
            else if (cloud.partial)
                setN("cr", "partial installation detected — repair required");
        }
        catch { /* */ }
        try {
            const s = await systemStatus();
...
function formatSize(bytes, local = true) {
    if (!local)
        return "Cloud only";
    if (!bytes)
        return "No local save files yet";
    const units = ["B", "KB", "MB", "GB"];
    const index = Math.min(Math.floor(Math.log(bytes) / Math.log(1024)), units.length - 1);
    const value = bytes / Math.pow(1024, index);
    return `${value >= 10 || index === 0 ? value.toFixed(0) : value.toFixed(1)} ${units[index]}`;
}
function formatRemoteSave(timestamp, provider, remote = false) {
    const providerName = provider === "gdrive" ? "Google Drive" : provider === "onedrive" ? "OneDrive" :
        provider === "folder" ? "custom folder" : "local storage";
    if (!timestamp)
        return provider === "local" ? "No stored save metadata yet" :
...
function steamGame(appid, resolvedName = "") {
    let title = resolvedName || `Steam App ${appid}`;
    try {
        const overview = window.appStore?.GetAppOverviewByAppID?.(appid)
            || window.appStore?.GetAppOverviewByGameID?.(appid);
...
function CloudSaveCard({ app, provider }) {
    const game = steamGame(app.appid, app.name);
    const [artIndex, setArtIndex] = SP_REACT.useState(0);
    const [localArtwork, setLocalArtwork] = SP_REACT.useState("");
    const [localArtworkChecked, setLocalArtworkChecked] = SP_REACT.useState(false);
    ...
    const openGame = () => {
        try {
            DFL.Navigation.Navigate(`/library/app/${app.appid}`);
            DFL.Navigation.CloseSideMenus?.();
        }
        catch { /* Steam may not have finished loading this app overview yet */ }
    };
    return SP_JSX.jsxs(DFL.DialogButton, { onClick: openGame, style: {
        ...
        }, children: [artwork && SP_JSX.jsx("img", { src: artwork, alt: "", onError: advanceArtwork, style: {
                    position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover",
                    opacity: .48,
                } }), SP_JSX.jsx("div", { style: {
                    position: "absolute", inset: 0,
                    background: "linear-gradient(90deg, rgba(8, 16, 25, .96) 0%, rgba(8, 16, 25, .76) 54%, rgba(8, 16, 25, .28) 100%)",
                } }), SP_JSX.jsxs("div", { style: { position: "relative", padding: "13px 14px", textShadow: "0 1px 3px #000" }, children: [SP_JSX.jsx("div", { style: { fontSize: 16, fontWeight: 700, lineHeight: 1.18 }, children: game.title }), SP_JSX.jsxs("div", { style: { display: "flex", flexWrap: "wrap", gap: "5px 12px", marginTop: 13, fontSize: 11 }, children: [SP_JSX.jsx("span", { style: { color: "#67c1f5", fontWeight: 650 }, children: formatSize(app.size, app.local !== false) }), SP_JSX.jsx("span", { style: { opacity: .82 }, children: app.local === false ? "Files available remotely" : `${app.files} ${app.files === 1 ? "file" : "files"}` }), app.remote && app.local !== false && SP_JSX.jsx("span", { style: { color: "#5ee6c4", fontWeight: 650 }, children: "Local + cloud" }), SP_JSX.jsx("span", { style: { opacity: .82 }, children: formatRemoteSave(app.remoteTime, provider, !!app.remote) })] })] })] });
}
...
function CloudRedirectSection() {
    ...
    const [saves, setSaves] = SP_REACT.useState([]);
    ...
    const watchAutomaticCallback = async (watchId) => {
    ...
    const saveFolder = async () => {
        ...
        try {
            ...
            setState(result);
            if (!result.success)
                throw new Error(result.error || "Could not use that folder");
            ...
            setMsg(`Folder setup failed: ${error}`);
        }
        if (alive.current)
            setBusy(false);
    };
    ...
    const finishCallback = async () => {
        if (!callbackUrl.trim())
            return;
        setBusy(true);
        setMsg("Completing CloudRedirect sign-in…");
        try {
    ...
    const sortedSaves = [...saves].sort((a, b) => {
        ...
        if (aHasSaves !== bHasSaves)
            return aHasSaves ? -1 : 1;
        if ((b.remoteTime || 0) !== (a.remoteTime || 0))
            return (b.remoteTime || 0) - (a.remoteTime || 0);
...
function deckyBackend() {
    return window.DeckyBackend ?? window.opener?.DeckyBackend ?? null;
}
/**
...
function UpdatesSection() {
    ...
    const [plugin, setPlugin] = SP_REACT.useState(null);
    const [releases, setReleases] = SP_REACT.useState([]);
    const [selectedChannel, setSelectedChannel] = SP_REACT.useState("update-system");
    const [selectedTag, setSelectedTag] = SP_REACT.useState("");
    const [pluginBusy, setPluginBusy] = SP_REACT.useState(false);
    const [pluginProgress, setPluginProgress] = SP_REACT.useState(0);
    const [pluginMsg, setPluginMsg] = SP_REACT.useState("");
    const [updateBanners, setUpdateBanners] = SP_REACT.useState(true);
    const pluginDownloadStarted = SP_REACT.useRef(false);
    ...
    const load = async () => {
        ...
        try {
            ...
            setPluginMsg(`Plugin update check failed: ${error}`);
        }
        try {
    ...
    SP_REACT.useEffect(() => {
        ...
        if (!backend?.addEventListener)
            return;
        const start = (name) => {
            if (name !== "SLSDeckUniversal")
                return;
            pluginDownloadStarted.current = true;
            setPluginBusy(true);
            setPluginProgress(0);
            setPluginMsg("Downloading plugin…");
        };
        ...
        const finish = (name) => {
            ...
            setPluginMsg("Plugin installed. Reloading…");
            setPluginBusy(false);
    ...
    const installSelected = async () => {
        ...
        try {
            ...
            if (!armed.success)
                throw new Error(armed.error || "Could not arm safe replacement");
            await backend.call("utilities/install_plugin", selectedRelease.assetUrl, "SLSDeckUniversal", selectedRelease.version, "", installType);
            ...
            window.setTimeout(() => {
                if (!pluginDownloadStarted.current)
                    setPluginBusy(false);
            }, 5000);
        }
        catch (error) {
    ...
    return (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsxs(DFL.PanelSection, { title: "SLSDeck plugin updates", children: [SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsxs("div", { style: { fontSize: 12, lineHeight: 1.5, width: "100%" }, children: [SP_JSX.jsxs("div", { children: ["Installed: ", SP_JSX.jsx("b", { children: plugin?.currentVersion || "checking…" })] }), SP_JSX.jsxs("div", { children: ["Channel: ", SP_JSX.jsx("b", { children: plugin?.currentChannel || "unknown" })] }), SP_JSX.jsx("div", { style: { opacity: 0.72 }, children: plugin?.updateAvailable
                                        ? `Update available: ${plugin.latest?.version}`
                                        : plugin?.success ? "This update channel is current." : (plugin?.error || "Checking GitHub releases…") })] }) }), channels.length > 0 && SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "Release channel", description: "Switch to a prebuilt rolling release from another branch. Branches without this updater may require manually reinstalling update-system to return.", rgOptions: channels.map((channel) => ({ data: channel, label: channel })), selectedOption: selectedChannel, strDefaultLabel: selectedChannel || "Choose a channel", onChange: (option) => setSelectedChannel(String(option.data || "")), disabled: pluginBusy }) }), releases.length > 0 && SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "Install version", description: selectedChannel === "update-system"
                                ? "Choose rolling latest or an immutable historical build. Managed dependencies and user data are preserved."
                                : "This channel currently publishes only its prebuilt rolling latest release.", rgOptions: channelReleases.map((item) => ({ data: item.tag, label: item.version })), selectedOption: selectedRelease?.tag || "", strDefaultLabel: selectedRelease?.version || "Choose a build", onChange: (option) => setSelectedTag(String(option.data || "")), disabled: pluginBusy }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ButtonItem, { layout: "below", onClick: installSelected, disabled: pluginBusy || !selectedRelease, children: selectedRelease
...
function BackupSection() {
    ...
    const [fullPurge, setFullPurge] = SP_REACT.useState(false);
    const [busy, setBusy] = SP_REACT.useState(false);
    ...
    return (SP_JSX.jsxs(DFL.PanelSection, { title: "Backup & restore", children: [SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ToggleField, { label: "Include API keys", description: "Off (default): your saved API keys are stripped from the export. On: keys are included \u2014 keep the file private.", checked: includeKeys, disabled: busy, onChange: setIncludeKeys }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ToggleField, { label: "Backup local game saves", description: "On (default): also back up each installed SLSDeck game's Proton-prefix saves (AppData, Saved Games, Documents). Can make the archive large.", checked: includeSaves, disabled: busy, onChange: setIncludeSaves }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ToggleField, { label: "Full purge on uninstall", description: "Off (default): preserve CloudRedirect saves, provider data, SLSsteam configuration and added-game records. On: uninstalling SLSDeck through Decky permanently removes all managed dependencies and data.", checked: fullPurge, disabled: busy, onChange: (enabled) => {
...
const ROULETTE_QAM_KEY = "slsdeck.storeRoulette.quickAccess";
const ROULETTE_TAB_DISABLED_KEY = "slsdeck.storeRoulette.tabDisabled";
const ROULETTE_PRICE_KEY = "slsdeck.storeRoulette.minPrice";
const ROULETTE_FILTERS_KEY = "slsdeck.storeRoulette.filters";
const ROULETTE_PREFS_EVENT = "slsdeck-store-roulette-prefs";
const DEFAULT_ROULETTE_FILTERS = {
    priceEnabled: false,
    priceDirection: "min",
    priceCents: 6000,
    qualityMode: false,
...
function writeRouletteBool(key, value) {
    try {
...
function readRouletteFilters() {
    try {
        ...
        return {
            ...
            priceCents: Math.max(100, Math.min(100000, Number(hasSliderPrice ? parsed.priceCents : legacyPrice) || DEFAULT_ROULETTE_FILTERS.priceCents)),
        ...
        return { ...DEFAULT_ROULETTE_FILTERS };
    }
}
function writeRouletteFilters(value) {
    try {
        ...
        window.localStorage.setItem(ROULETTE_PRICE_KEY, String(value.priceEnabled && value.priceDirection === "min" ? value.priceCents : 0));
...
function GameArtwork({ item }) {
    ...
    if (sourceIndex >= sources.length)
        return SP_JSX.jsx("div", { style: {
            ...
            }, children: item.name || "Steam game" });
    return SP_JSX.jsx("img", { src: sources[sourceIndex], alt: "", onError: () => setSourceIndex((index) => index + 1), style: { display: "block", width: "100%", height: "100%", objectFit: "contain" } });
}
function WinnerRevealModal({ item, onReturn, closeModal }) {
    const gamepadFrame = SP_REACT.useRef(0);
    const [dismissing, setDismissing] = SP_REACT.useState(false);
    const dismiss = () => {
        if (dismissing)
            return;
        setDismissing(true);
    ...
    SP_REACT.useEffect(() => {
        ...
        const dismissWhenArmed = () => { if (armed)
            dismiss(); };
        ...
        const watch = () => {
            ...
            if (armed && pads.some((pad, pi) => pad?.buttons.some((button, bi) => button.pressed && !previous[pi]?.[bi])))
                dismiss();
            else {
    ...
    return SP_JSX.jsxs(DFL.ModalRoot, { closeModal: dismiss, onCancel: dismiss, bHideCloseIcon: true, className: "sls-winner-content", modalClassName: "sls-winner-shell", children: [SP_JSX.jsx("style", { children: `
      .sls-winner-shell {
    ...
    ` }), SP_JSX.jsx("div", { onPointerDown: dismiss, style: { position: "relative", zIndex: 2, width: "min(calc(100vw - 64px),430px)" }, children: SP_JSX.jsx("div", { style: {
                        width: "100%", textAlign: "center",
                        animation: dismissing ? "sls-winner-exit 300ms ease-in both" : "sls-winner-enter 850ms cubic-bezier(.18,.82,.2,1) both",
                    }, children: SP_JSX.jsxs("div", { style: { animation: "sls-winner-idle 3s ease-in-out 1s infinite" }, children: [SP_JSX.jsxs("div", { style: { position: "relative", width: "100%", aspectRatio: "16 / 9", overflow: "hidden", borderRadius: 12, background: "linear-gradient(145deg,#23354a,#111c29)", boxShadow: "0 24px 58px rgba(0,0,0,.82),0 0 44px rgba(255,190,75,.48)" }, children: [SP_JSX.jsx(GameArtwork, { item: item }), SP_JSX.jsx("div", { style: { position: "absolute", inset: "0 auto 0 0", width: "36%", animation: "sls-winner-shine 1.8s ease-in-out 600ms both", background: "linear-gradient(90deg,transparent,rgba(255,255,255,.62),transparent)", filter: "blur(2px)" } })] }), SP_JSX.jsx("div", { style: { marginTop: 16, color: "#ffc95c", fontSize: 13, fontWeight: 900, letterSpacing: 1.6 }, children: "WILL BE ADDED TO YOUR LIBRARY" }), SP_JSX.jsx("div", { style: { marginTop: 5, fontSize: 23, fontWeight: 900, textShadow: "0 3px 12px #000" }, children: item.name }), SP_JSX.jsxs("div", { style: { marginTop: 8, color: "#a7e7bb", fontSize: 17, fontWeight: 900, textShadow: "0 2px 9px #000" }, children: ["SAVED ", displayPrice(item)] })] }) }) })] });
}
...
async function addWinnerToSlsSteam(item) {
    const started = await startAdd(item.appid);
    if (!started.success)
        throw new Error(started.error || "SLS Steam could not start adding the winner");
...
function MinigameSection({ modalClose, onBusyChange, quickAccess = false } = {}) {
    const viewport = SP_REACT.useRef(null);
    const animation = SP_REACT.useRef(0);
    const gamepadWatch = SP_REACT.useRef(0);
    const tabDismissTimer = SP_REACT.useRef(0);
    const tabDismissingRef = SP_REACT.useRef(false);
    const caseSounds = SP_REACT.useRef([]);
    const prefetchQueue = SP_REACT.useRef([]);
    const prefetchGeneration = SP_REACT.useRef(0);
    const prefetchRunningGeneration = SP_REACT.useRef(null);
    const rolledAppids = SP_REACT.useRef(new Set());
    const [items, setItems] = SP_REACT.useState([]);
    const [offset, setOffset] = SP_REACT.useState(0);
    const [busy, setBusy] = SP_REACT.useState(false);
    const [winner, setWinner] = SP_REACT.useState();
    const [winnerAdded, setWinnerAdded] = SP_REACT.useState(false);
    const [error, setError] = SP_REACT.useState("");
    const [filters, setFilters] = SP_REACT.useState(readRouletteFilters);
    const [filtersExpanded, setFiltersExpanded] = SP_REACT.useState(false);
    const [revealVisible, setRevealVisible] = SP_REACT.useState(false);
    const [tabRevealDismissing, setTabRevealDismissing] = SP_REACT.useState(false);
    const [returningFromWinner, setReturningFromWinner] = SP_REACT.useState(false);
    ...
    const fillPrefetchQueue = async (generation = prefetchGeneration.current) => {
        if (prefetchRunningGeneration.current === generation || generation !== prefetchGeneration.current)
            return;
        ...
        try {
            while (generation === prefetchGeneration.current && prefetchQueue.current.length < PREFETCH_DEPTH) {
                ...
                if (generation !== prefetchGeneration.current)
                    break;
                if (!isUsableRoll(result))
                    break;
                if (rolledAppids.current.has(result.winner.appid)
                    || prefetchQueue.current.some((queued) => queued.winner?.appid === result.winner.appid))
                    continue;
                prefetchQueue.current.push(result);
            }
        }
        finally {
            if (prefetchRunningGeneration.current === generation)
                prefetchRunningGeneration.current = null;
        }
    };
    const changeFilter = (key, value) => {
        ...
        if (next.releaseFrom && next.releaseTo && next.releaseFrom > next.releaseTo) {
            if (key === "releaseFrom")
                next.releaseTo = next.releaseFrom;
            else
                next.releaseFrom = next.releaseTo;
        }
        setFilters(next);
        writeRouletteFilters(next);
        setError("");
    };
    const dismissTabReveal = () => {
        if (tabDismissingRef.current)
            return;
    ...
    SP_REACT.useEffect(() => {
        ...
        return () => {
            clearTimeout(timer);
            if (prefetchGeneration.current === generation)
    ...
    SP_REACT.useEffect(() => {
        ...
        const watchedEvents = ["keydown", "pointerdown", "mousedown", "click", "touchstart", "touchend"];
        watchedEvents.forEach((name) => document.addEventListener(name, dismiss, true));
        let previousButtons = Array.from(navigator.getGamepads?.() || []).map((pad) => pad ? pad.buttons.map((button) => button.pressed) : []);
        const watchGamepad = () => {
            const pads = Array.from(navigator.getGamepads?.() || []);
            const newlyPressed = pads.some((pad, padIndex) => pad?.buttons.some((button, buttonIndex) => button.pressed && !previousButtons[padIndex]?.[buttonIndex]));
            if (newlyPressed)
                dismiss();
            else {
    ...
    const openWinner = () => {
        ...
        try {
            if (winnerAdded)
                DFL.Navigation.Navigate(`/library/app/${winner.appid}`);
            else
                DFL.Navigation.NavigateToExternalWeb(`https://store.steampowered.com/app/${winner.appid}`);
        }
        catch { /* ignore */ }
    };
    const roll = async () => {
        ...
        try {
            ...
            const addResult = addWinnerToSlsSteam(result.winner).then(() => ({ success: true }), (cause) => ({ success: false, error: String(cause?.message || cause) }));
            ...
            const frame = (now) => {
                ...
                if (progress < 1)
                    ...
                    if (quickAccess) {
                        setRevealVisible(false);
                        DFL.showModal(SP_JSX.jsx(WinnerRevealModal, { item: result.winner, onReturn: () => {
    ...
    const rouletteWindow = SP_JSX.jsxs("div", { ref: viewport, style: {
            ...
            animation: returningFromWinner ? "sls-roulette-return 480ms ease-out both" : undefined,
        }, children: [SP_JSX.jsx("style", { children: `@keyframes sls-roulette-return { from { opacity:.58; filter:blur(5px) brightness(.68); } to { opacity:1; filter:blur(0) brightness(1); } }` }), SP_JSX.jsx("div", { style: { position: "absolute", zIndex: 4, left: "50%", top: 0, bottom: 0, width: 2, transform: "translateX(-1px)", background: "linear-gradient(#ffd86a, #ff9c32, #ffd86a)", boxShadow: "0 0 13px #ffb23f" } }), SP_JSX.jsx("div", { style: { position: "absolute", zIndex: 5, left: "50%", top: 0, transform: "translateX(-50%)", width: 0, height: 0, borderLeft: "8px solid transparent", borderRight: "8px solid transparent", borderTop: "11px solid #ffd86a" } }), !items.length && busy && SP_JSX.jsx("div", { style: { position: "absolute", zIndex: 3, inset: 0, display: "grid", placeItems: "center", textAlign: "center", background: quickAccess ? "rgba(5,10,17,.58)" : "transparent", letterSpacing: .5 }, children: "LOADING THE STEAM STORE\u2026" }), !quickAccess && !items.length && !busy && SP_JSX.jsx("div", { style: { height: "100%", display: "grid", placeItems: "center", textAlign: "center", opacity: .62, letterSpacing: .5 }, children: "A RANDOM GAME AWAITS" }), SP_JSX.jsxs("div", { style: { display: "flex", gap: CARD_GAP, height: "100%", padding: "10px 0", transform: `translate3d(${-offset}px,0,0)`, willChange: "transform" }, children: [quickAccess && !items.length && PLACEHOLDER_CARDS.map((label, index) => SP_JSX.jsx("div", { style: {
                        ...
                        }, children: label }, `placeholder-${index}`)), items.map((item, index) => SP_JSX.jsx("div", { style: {
                            position: "relative", flex: `0 0 ${CARD_WIDTH}px`, height: quickAccess ? 190 : 140, overflow: "hidden", borderRadius: 8,
                        ...
                        }, children: quickAccess ? SP_JSX.jsx(GameArtwork, { item: item }) : SP_JSX.jsx("img", { src: item.image, alt: "", onError: (event) => { event.currentTarget.style.display = "none"; }, style: { width: "100%", height: "100%", objectFit: "contain", opacity: .92 } }) }, `${item.appid}-${index}`))] })] });
    return SP_JSX.jsxs(SP_JSX.Fragment, { children: [!quickAccess && winner && revealVisible && SP_JSX.jsx("div", { style: {
                ...
                }, onPointerDown: dismissTabReveal, onTouchStart: dismissTabReveal, onClick: dismissTabReveal, children: SP_JSX.jsxs("div", { style: { width: "min(72vw, 430px)", textAlign: "center", position: "relative", animation: tabRevealDismissing ? "sls-tab-card-exit 300ms ease-in both" : undefined }, children: [SP_JSX.jsx("style", { children: `
      ...
      ` }), SP_JSX.jsxs("div", { children: [SP_JSX.jsxs("div", { style: { position: "relative", width: "100%", minHeight: 240, display: "grid", placeItems: "center" }, children: [SP_JSX.jsx("div", { style: { position: "absolute", left: "50%", top: "50%", width: 310, height: 310, borderRadius: "50%", animation: "sls-unlock-burst 1.15s ease-out both", background: "repeating-conic-gradient(from 0deg, rgba(255,202,87,.75) 0deg 5deg, transparent 5deg 17deg)", filter: "blur(1px)" } }), SP_JSX.jsx("div", { style: { position: "absolute", inset: 0, borderRadius: "50%", animation: "sls-unlock-flash 900ms ease-out both", background: "radial-gradient(circle,rgba(255,246,202,.85),rgba(255,193,73,.24) 35%,transparent 68%)" } }), Array.from({ length: 12 }, (_, index) => SP_JSX.jsx("span", { style: { position: "absolute", left: "50%", top: "50%", width: 4, height: 4, transform: `rotate(${index * 30}deg)`, transformOrigin: "0 0" }, children: SP_JSX.jsx("span", { style: { display: "block", width: index % 3 === 0 ? 7 : 4, height: index % 3 === 0 ? 7 : 4, borderRadius: index % 2 ? "50%" : 1, background: index % 2 ? "#fff1a4" : "#ffad3d", boxShadow: "0 0 8px #ffc45d", animation: `sls-unlock-particle ${760 + (index % 4) * 90}ms ease-out ${index * 24}ms both` } }) }, index)), SP_JSX.jsx("div", { style: { position: "relative", zIndex: 2, display: "inline-block", maxWidth: "76%", borderRadius: 10, overflow: "hidden", animation: "sls-game-unlocked 820ms cubic-bezier(.18,.82,.2,1) both", boxShadow: "0 22px 48px rgba(0,0,0,.72), 0 0 40px rgba(255,190,75,.38)" }, children: SP_JSX.jsxs("div", { style: { position: "relative", animation: "sls-cover-float 3s ease-in-out 1.15s infinite" }, children: [SP_JSX.jsx("img", { src: `https://cdn.cloudflare.steamstatic.com/steam/apps/${winner.appid}/library_600x900_2x.jpg`, alt: "", onError: (event) => { if (event.currentTarget.dataset.fallback)
                                                            return; event.currentTarget.dataset.fallback = "1"; event.currentTarget.src = winner.image; }, style: { display: "block", width: "100%", maxHeight: 330, objectFit: "contain" } }), SP_JSX.jsx("div", { style: { position: "absolute", zIndex: 3, top: 0, bottom: 0, left: 0, width: "38%", animation: "sls-cover-shine 1.8s ease-in-out 700ms both", background: "linear-gradient(90deg,transparent,rgba(255,255,255,.65),transparent)", filter: "blur(2px)" } })] }) })] }), SP_JSX.jsx("div", { style: { fontSize: 12, color: "#ffc95c", fontWeight: 900, letterSpacing: 1.5, animation: "sls-unlock-title 850ms ease-out 250ms both" }, children: "GAME UNLOCKED" }), SP_JSX.jsx("div", { style: { fontSize: 23, fontWeight: 900, marginTop: 5, textShadow: "0 3px 12px #000" }, children: winner.name }), SP_JSX.jsxs("div", { style: { fontSize: 17, color: "#a7e7bb", fontWeight: 900, marginTop: 8, textShadow: "0 2px 9px #000" }, children: ["SAVED ", displayPrice(winner)] })] })] }) }), quickAccess ? SP_JSX.jsxs("div", { style: {
                    position: "fixed", zIndex: 10000, left: "50%", top: "50%", transform: "translate(-50%, -50%)",
                    width: "min(76vw, 820px)", margin: 0,
                }, children: [rouletteWindow, error && SP_JSX.jsx("div", { style: { color: "#ff8b83", fontSize: 11, marginTop: 8 }, children: error }), SP_JSX.jsx(DFL.DialogButton, { disabled: busy, onClick: roll, style: { width: "100%", marginTop: 10 }, children: busy ? "Rolling…" : "Roll a game" })] }) : SP_JSX.jsxs(DFL.PanelSection, { title: "Store Roulette", children: [SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsxs(DFL.ButtonItem, { layout: "below", onClick: () => setFiltersExpanded((expanded) => !expanded), children: ["Roulette filters \u00B7 ", activeFilterCount ? `${activeFilterCount} active` : "Random", " ", filtersExpanded ? "▲" : "▼"] }) }), filtersExpanded && SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ToggleField, { label: "Price filter", description: filters.priceEnabled
                                        ? `${filters.priceDirection === "min" ? "At least" : "At most"} $${filters.priceCents / 100}${filters.priceDirection === "min" && filters.priceCents === 100000 ? "+" : ""}`
                                        : "Off keeps price random", checked: filters.priceEnabled, onChange: (value) => changeFilter("priceEnabled", value) }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "Price rule", rgOptions: PRICE_DIRECTION_OPTIONS, disabled: !filters.priceEnabled, selectedOption: filters.priceDirection, strDefaultLabel: filters.priceDirection === "max" ? "At most" : "At least", onChange: (option) => changeFilter("priceDirection", option.data === "max" ? "max" : "min") }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.SliderField, { label: "Price threshold", description: filters.priceCents === 100000
                                        ? (filters.priceDirection === "min" ? "$1000+ endpoint" : "$1000 maximum")
                                        : `${filters.priceDirection === "min" ? "Games costing at least" : "Games costing at most"} $${filters.priceCents / 100}`, value: Math.round(filters.priceCents / 100), min: 1, max: 1000, step: 1, notchCount: 5, notchTicksVisible: true, showValue: true, editableValue: true, valueSuffix: " USD", minimumDpadGranularity: 1, disabled: !filters.priceEnabled, onChange: (value) => changeFilter("priceCents", Math.max(1, Math.min(1000, Math.round(value))) * 100) }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ToggleField, { label: "Quality mode", description: "Requires at least 300 reviews and 60% positive ratings. This overrides the two manual review filters while enabled.", checked: filters.qualityMode, onChange: (value) => changeFilter("qualityMode", value) }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ToggleField, { label: "Use Steam personalization (experimental)", description: "Asks Steam Store search to respect recognized account preferences. Adds to every selected filter; results may remain neutral when Steam does not expose its signed-in session to the plugin backend.", checked: filters.personalized, onChange: (value) => changeFilter("personalized", value) }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "Genre", rgOptions: GENRE_OPTIONS, selectedOption: filters.genre, strDefaultLabel: GENRE_OPTIONS.find((option) => option.data === filters.genre)?.label || "Any genre", onChange: (option) => changeFilter("genre", String(option.data || "")) }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "Players", rgOptions: PLAYER_OPTIONS, selectedOption: filters.players, strDefaultLabel: PLAYER_OPTIONS.find((option) => option.data === filters.players)?.label || "Any player mode", onChange: (option) => changeFilter("players", String(option.data || "")) }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "Steam Deck", rgOptions: DECK_OPTIONS, selectedOption: filters.deck, strDefaultLabel: DECK_OPTIONS.find((option) => option.data === filters.deck)?.label || "Any Deck status", onChange: (option) => changeFilter("deck", String(option.data || "")) }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "User rating", rgOptions: RATING_OPTIONS, disabled: filters.qualityMode, description: filters.qualityMode ? "Controlled by Quality mode: 60%+ positive" : "Optional minimum positive rating", selectedOption: filters.minRating, strDefaultLabel: RATING_OPTIONS.find((option) => option.data === filters.minRating)?.label || "Any rating", onChange: (option) => changeFilter("minRating", Number(option.data) || 0) }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "Review count", rgOptions: REVIEW_OPTIONS, disabled: filters.qualityMode, description: filters.qualityMode ? "Controlled by Quality mode: 300+ reviews" : "Optional minimum number of reviews", selectedOption: filters.minReviews, strDefaultLabel: REVIEW_OPTIONS.find((option) => option.data === filters.minReviews)?.label || "Any review count", onChange: (option) => changeFilter("minReviews", Number(option.data) || 0) }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "Released from", rgOptions: YEAR_OPTIONS, selectedOption: filters.releaseFrom, strDefaultLabel: filters.releaseFrom ? String(filters.releaseFrom) : "Any year", onChange: (option) => changeFilter("releaseFrom", Number(option.data) || 0) }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "Released through", rgOptions: YEAR_OPTIONS, selectedOption: filters.releaseTo, strDefaultLabel: filters.releaseTo ? String(filters.releaseTo) : "Any year", onChange: (option) => changeFilter("releaseTo", Number(option.data) || 0) }) }), activeFilterCount > 0 && SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ButtonItem, { layout: "below", onClick: () => {
                                        const cleared = { ...DEFAULT_ROULETTE_FILTERS };
                                        setFilters(cleared);
                                        writeRouletteFilters(cleared);
                                        setError("");
                                    }, children: "Clear roulette filters" }) })] }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx("div", { style: { fontSize: 11, opacity: .72, lineHeight: 1.45 }, children: "Crack open the entire Steam Store. Games already in your library are excluded, and the winner is automatically added with SLS Steam." }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: rouletteWindow }), error && SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx("div", { style: { color: "#ff8b83", fontSize: 11 }, children: error }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ButtonItem, { layout: "below", disabled: busy, onClick: roll, children: busy ? "Rolling…" : "Roll a game" }) }), winner && SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsxs(DFL.ButtonItem, { layout: "below", onClick: openWinner, children: ["View ", winner.name, " in ", winnerAdded ? "Library" : "Store"] }) })] })] });
}
function StoreRouletteModal({ closeModal }) {
    const [busy, setBusy] = SP_REACT.useState(false);
    const close = () => { if (!busy)
        closeModal?.(); };
    return (SP_JSX.jsxs(DFL.ModalRoot, { closeModal: close, onCancel: close, onEscKeypress: close, bDisableBackgroundDismiss: busy, bHideCloseIcon: true, bAllowFullSize: true, className: "sls-roulette-modal", modalClassName: "sls-roulette-modal", children: [SP_JSX.jsx("style", { children: `
        .sls-roulette-modal { background: transparent !important; box-shadow: none !important; border: 0 !important; }
      ` }), SP_JSX.jsx(MinigameSection, { modalClose: closeModal, onBusyChange: setBusy, quickAccess: true })] }));
}
...
function AddDownloadToggle() {
    ...
    const [reloadOnPurge, setReloadOnPurgeState] = SP_REACT.useState(false);
    SP_REACT.useEffect(() => {
        getAutoDownload().then((r) => setOn(!!r.enabled)).catch(() => { });
        getReloadOnPurge().then((r) => setReloadOnPurgeState(!!r.enabled)).catch(() => { });
    }, []);
    return (SP_JSX.jsxs(DFL.PanelSection, { title: "Adding games", children: [SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ToggleField, { label: "Auto restart after adding", description: "Auto restart Steam after adding a game.", checked: on, onChange: async (v) => { setOn(v); await setAutoDownload(v); } }) }), SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.ToggleField, { label: "Reload when purging all", description: "Compatibility fallback that restarts Steam after Purge All. Off uses Moon's in-session hot removal.", checked: reloadOnPurge, onChange: async (v) => { setReloadOnPurgeState(v); await setReloadOnPurge(v); } }) })] }));
}
function DlcCloudToggles() {
    const [dlc, setDlc] = SP_REACT.useState(true);
    const [dlcOwnedOnly, setDlcOwnedOnlyState] = SP_REACT.useState(true);
    const [autoDlc, setAutoDlc] = SP_REACT.useState(true);
...
function OptionsPane({ showDeckyHv, onShowDeckyHvChange, }) {
    ...
    const [rouletteTabDisabled, setRouletteTabDisabled] = SP_REACT.useState(() => readRouletteBool(ROULETTE_TAB_DISABLED_KEY));
    SP_REACT.useEffect(() => {
...
function AdvancedPage() {
    ...
    return (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx("style", { children: ADVANCED_PAGE_THEME }), SP_JSX.jsx(DFL.SidebarNavigation, { title: "SLSDeck", showTitle: true, pages: [
                    ...
                    ...(!rouletteTabDisabled ? [{
                            ...
                            content: SP_JSX.jsx(Body, { children: SP_JSX.jsx(MinigameSection, {}) }),
                        }] : []),
                    {
                        title: "About",
...
async function cleanupLegacyCloudRedirectShortcut() {
    try {
        const [{ appId }, status] = await Promise.all([crGetShortcut(), crInstallStatus()]);
        const id = Number(appId || 0);
        if (!id || status?.legacyFlatpakInstalled || status?.appInstalled)
            return;
        const SC = window.SteamClient;
        if (!SC?.Apps?.RemoveShortcut)
            return;
...
function readCachedSlsStatus() {
    try {
        const value = JSON.parse(window.localStorage.getItem(SLS_STATUS_CACHE_KEY) || "null");
        return value && typeof value.installed === "boolean" ? value : null;
    }
    catch {
        return null;
    }
}
function writeCachedSlsStatus(status) {
    try {
        window.localStorage.setItem(SLS_STATUS_CACHE_KEY, JSON.stringify(status));
    }
    catch { /* ignore */ }
}
...
function QamTitle() {
    const [rouletteEnabled, setRouletteEnabled] = SP_REACT.useState(() => readRouletteBool(ROULETTE_QAM_KEY));
    SP_REACT.useEffect(() => {
    ...
    return (SP_JSX.jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }, children: [SP_JSX.jsx("div", { className: DFL.staticClasses.Title, children: "SLSDeck" }), SP_JSX.jsxs("div", { style: { display: "flex", gap: 5 }, children: [rouletteEnabled && SP_JSX.jsx(DFL.DialogButton, { onClick: () => DFL.showModal(SP_JSX.jsx(StoreRouletteModal, {})), style: buttonStyle, "aria-label": "Open Store Roulette", children: SP_JSX.jsx(FaDice, {}) }), SP_JSX.jsx(DFL.DialogButton, { onClick: () => {
                            try {
                                DFL.Navigation.CloseSideMenus();
                        ...
                        }, style: buttonStyle, "aria-label": "Advanced settings", children: SP_JSX.jsx(FaCog, {}) })] })] }));
}
const PURGE_ADDED_GAMES_EVENT = "slsdeck-purge-added-games";
const purgedAtByAppId = new Map();
function readPendingAddVerifications() {
    try {
...
function writePendingAddVerifications(items) {
    try {
        window.localStorage.setItem(PENDING_ADD_VERIFY_KEY, JSON.stringify(items.slice(-50)));
    }
    catch { /* ignore */ }
}
function queueAddVerification(appid, name, liveReady) {
...
function steamLibraryHasApp(appid) {
    let libraryCollectionAvailable = false;
    try {
        const apps = window.collectionStore?.allAppsCollection?.apps;
        if (apps) {
            libraryCollectionAvailable = true;
            if (apps?.has?.(appid) || apps?.has?.(String(appid)) || apps?.get?.(appid) || apps?.get?.(String(appid)))
                return true;
        }
    }
    catch { /* ignore */ }
    try {
        if (window.appStore?.GetAppOverviewByAppID?.(appid) ||
            window.appStore?.GetAppOverviewByGameID?.(appid))
            return true;
    }
    catch { /* ignore */ }
...
async function verifyAddedGameReachedSteam(appid, name, createdAt = Date.now()) {
...
async function verifyAddsPendingFromPreviousSteamSession() {
...
function Content() {
    ...
    const [slsStatus, setSlsStatus] = SP_REACT.useState(() => readCachedSlsStatus());
    const [slsStatusChecked, setSlsStatusChecked] = SP_REACT.useState(false);
    const [pluginUpdate, setPluginUpdate] = SP_REACT.useState(null);
    const [updateBanners, setUpdateBanners] = SP_REACT.useState(true);
    const installed = slsStatus?.installed === true;
    const refreshSlsStatus = SP_REACT.useCallback(async () => {
        try {
    ...
    SP_REACT.useEffect(() => {
        ...
        getUiSettings().then((result) => {
            if (active)
                setUpdateBanners(result.settings?.pluginUpdateBanners !== false);
        }).catch(() => { });
        const onBannerSetting = (rawEvent) => {
            const event = rawEvent;
            setUpdateBanners(event.detail !== false);
        };
        window.addEventListener(UPDATE_BANNERS_EVENT, onBannerSetting);
        void check();
        const interval = window.setInterval(check, 15 * 60 * 1000);
        return () => {
            active = false;
            window.clearInterval(interval);
            window.removeEventListener(UPDATE_BANNERS_EVENT, onBannerSetting);
        };
    }, []);
    ...
    return (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx("div", { ref: anchor, style: { height: 0 } }), updateBanners && pluginUpdate?.updateAvailable && pluginUpdate.latest && (SP_JSX.jsx(DFL.DialogButton, { onClick: () => {
                    try {
                        DFL.Navigation.CloseSideMenus();
                        DFL.Navigation.Navigate(ADVANCED_ROUTE);
                    }
                ...
                }, children: SP_JSX.jsxs("div", { style: { width: "100%", lineHeight: 1.35 }, children: [SP_JSX.jsx("div", { style: { fontSize: 14, fontWeight: 800 }, children: "SLSDeck update available" }), SP_JSX.jsxs("div", { style: { fontSize: 11, opacity: .9 }, children: [pluginUpdate.currentChannel, " \u2192 ", pluginUpdate.latest.version, " \u00B7 Open Advanced \u2192 Options"] })] }) })), SP_JSX.jsx(RepairBanner, {}), SP_JSX.jsx(SlsSteamCompact, { status: slsStatus, statusChecked: slsStatusChecked, onRefreshStatus: refreshSlsStatus }), installed && actionsFixesQam && SP_JSX.jsx(GameControlsSection, { onChanged: bump }), installed && SP_JSX.jsx(GameToolsSection, {}), installed && gamesInQam && SP_JSX.jsx(InstalledSection, { refreshToken: refreshToken, onChanged: bump }), installed && !hideToolsQam && SP_JSX.jsx(ToolsSection, {})] }));
}
var index = definePlugin(() => {
    ...
    const invalidatePurgedVerifications = (rawEvent) => {
        const event = rawEvent;
    ...
    const addNotifier = setInterval(async () => {
        try {
            ...
            if (events.some((event) => event.status === "done" && event.success && !event.assella)) {
                try {
                    notifySuccessfulAdds = (await getNotifyGameAdd()).enabled !== false;
                }
                catch { /* default on */ }
            }
            ...
            if (events.some((event) => (event.sourceFailures || []).length > 0)) {
                try {
                    surfaceFailedSources = (await getUiSettings()).settings?.toastOnSourceFailure === true;
                }
                catch { /* default off */ }
            }
            events.forEach((e) => {
                ...
                if (!skipDuplicate && !suppressSuccessfulSlsAdd)
                    toaster.toast({
                        title: "SLSDeck",
    ...
    return {
        ...
        onDismount() {
            ...
            try {
                window.removeEventListener(PURGE_ADDED_GAMES_EVENT, invalidatePurgedVerifications);
            }
            catch { /* ignore */ }
            try {
